...
class SalesInvoice(SellingController):
    ...
    def validate(self):
        ...
        self.verfiying_the_existence_of_the_budget(project = self.project, department = self.department, company = self.company)
    ...
    def generating_logs(self,project,department,company):
        ...
        for result in results:
            self.generate_logs(project_budget = result.name)
        
    def generate_logs(self,project_budget = None):
        if project_budget:
            # Fetch the Project Budget document
            project_budget_doc = frappe.get_doc("Project Budget", project_budget)
            
            log_entry = {
                "sales_invoice_id" : self.name,
                "net_total":self.total,
                "grand_total":self.grand_total,
                "taxes_and_charges_total":self.total_taxes_and_charges,
            }
            
            # Append the log entry to the revenue_logs child table
            project_budget_doc.append('revenue_logs', log_entry)
            
            # Save the Project Budget document to persist the log entry
            project_budget_doc.save()
            
            frappe.db.commit()
        
    def verfiying_the_existence_of_the_budget(self,project,department,company):
    ...
    def on_submit(self):
        ...
        self.generating_logs(project = self.project, department = self.department, company = self.company)
    ...
    def updating_the_budget_data_after_cancellation(self):
            # Step 1: Get the budget names associated with the Purchase Invoice
            budgets = frappe.db.sql("""
                SELECT DISTINCT(PB.name) AS "budget_name"
                FROM `tabRevenue wise Budget Logs` AS IBL
                INNER JOIN `tabProject Budget` AS PB
                ON PB.name = IBL.parent
                WHERE IBL.sales_invoice_id = %s
            ...
            for budget in budgets:
                ...
                for log in project_budget.revenue_logs:
                    if log.sales_invoice_id == self.name:
    ...
    def removing_logs(self, log, project_budget_name, entry_id):
        ...
        log_found = False
        for project_log in project_budget.revenue_logs:
            if project_log.sales_invoice_id == log.sales_invoice_id:  # Ensure we match the log by id
        ...
        if log_found:
            ...
            frappe.throw(f"No log found for Entry ID: {log.sales_invoice_id}")
