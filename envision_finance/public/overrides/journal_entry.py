...
class JournalEntry(AccountsController):
    ...
    def updating_the_budget(self):
        for account in self.accounts:
            if account.custom_update_project_budget_:
                budget_data = self.fetching_budget_data(account.custom_project_budget)
                for budget in budget_data:
                    apply_budget_on = account.custom_apply_budget_on

                    # Check if budget applies to "Item" or "Item Group"
                    if apply_budget_on == budget.apply_budget_on:
                        item_match = (apply_budget_on == "Item" and budget.item == account.custom_item)
                        item_group_match = (apply_budget_on == "Item Group" and budget.item == account.custom_item_group)

                        if item_match or item_group_match:
                            # Ensure both debit/credit and budget.current_budget are not None before comparison
                            debit = account.debit_in_account_currency or 0.00
                            credit = account.credit_in_account_currency or 0.00
                            current_budget = budget.current_budget or 0.00

                            # Check if the debit or credit exceeds the budget
                            if (debit > current_budget or credit > current_budget):
                                if budget.action_if_annual_budget_exceeded_on_journal_entry == "Stop":
                                    frappe.throw(
                                        title = "Budget Overlimit",
                                        msg = f"<b>Budget</b>: {budget.name}<br><b> Item: {budget.item} <br>Budget</b> for this item <b>{budget.item}</b> is <b>{budget.current_budget}</b>."
                                    )
                                    break
                                if budget.action_if_annual_budget_exceeded_on_journal_entry == "Warn":
                                    frappe.msgprint(
                                ...
                                # Log debit or credit based on account values
                                if debit != 0.00 and credit == 0.00:
                                    print(f"\n\n\n{account.custom_project_budget}\n{apply_budget_on}\n\n{account.custom_item or account.custom_item_group}")
                                    self.generating_log(account, "Debited")
                                    break
                                if debit == 0.00 and credit != 0.00:
                                    print(f"\n\n\n{account.custom_project_budget}\n{apply_budget_on}\n\n{account.custom_item or account.custom_item_group}")
                                    self.generating_log(account, "Credited")
                                    break


    def fetching_budget_data(self,budget):
        ...
        SELECT 
            ...
            BI.current_budget,
            PB.action_if_annual_budget_exceeded_on_journal_entry,
            PB.name
        FROM `tabBudget Items` AS BI
        INNER JOIN `tabProject Budget` AS PB 
            ON PB.name = BI.parent
        WHERE PB.name = "{budget}"
        AND PB.workflow_state = "Approved"
        AND PB.docstatus = 1
        AND PB.disable = 0
        
        """
        
        return frappe.db.sql(result_data_sql,as_dict = True)
        
    def generating_log(self, account, key):
        print(f"Budget: {account.custom_project_budget}")
        ...
        for i in project_budget.budgeted_items:
            # Handle both Item and Item Group matching properly
        ...
        # Log the journal entry in the transactional logs
        if key == "Debited":
            log_entry = {
                'entry_type': "Journal Entry",
                'entry_id': account.parent,
        ...
        project_budget.append('transactional_logs', log_entry)
        project_budget.save()
        frappe.db.commit()
    ...
    def on_cancel(self):
        ...
        super().on_cancel()
        self.updating_budget_on_cancellation()
